...
static int32_t readCgroupSubsystemFile(struct J9PortLibrary *portLibrary, FILE *file, char *value)
{
	...
	while (fgets(&value[pointer], 10, file) != NULL) {
		pointer = ftell(file);
		if ((size - pointer) < 10 ) {
			size =  size + 10;
		}
	...
	}fprintf(stderr, "size of value - %lu", sizeof(value));
...
j9sysinfo_cgroup_subsystem_iterator_next(struct J9PortLibrary *portLibrary, J9CgroupMetricIteratorState *state, J9CgroupMetricElement *cgroupElement)
{
	...
	switch (state->subsystemid) {
		case OMR_CGROUP_SUBSYSTEM_MEMORY :
			...
			FILE *cgroup_memory_file = NULL;
			rc = omrsysinfo_get_handle_cgroup_subsystem_file(OMR_CGROUP_SUBSYSTEM_MEMORY, j9CgroupMemoryMetricMap[state->count].metricFilename, &cgroup_memory_file);
			fprintf(stderr, "reached switch case rc = %d", rc);
			if (cgroup_memory_file == NULL) {
				fprintf(stderr, "\n1 file is null\n");
			}
			readCgroupSubsystemFile(portLibrary, cgroup_memory_file, cgroupElement->value);
		...
		case OMR_CGROUP_SUBSYSTEM_CPU :
			...
			FILE *cgroup_cpu_file = NULL;
			rc = omrsysinfo_get_handle_cgroup_subsystem_file(OMR_CGROUP_SUBSYSTEM_CPU, j9CgroupCpuMetricMap[state->count].metricFilename, &cgroup_cpu_file);
			fprintf(stderr, "reached switch case rc = %d", rc);
			if (cgroup_cpu_file == NULL) {
				fprintf(stderr, "\n2 file is null\n");
			}
			readCgroupSubsystemFile(portLibrary, cgroup_cpu_file, cgroupElement->value);
		...
		case OMR_CGROUP_SUBSYSTEM_CPUSET :
			...
			FILE *cgroup_cpuset_file = NULL;
			rc = omrsysinfo_get_handle_cgroup_subsystem_file(OMR_CGROUP_SUBSYSTEM_CPUSET, j9CgroupCpusetMetricMap[state->count].metricFilename, &cgroup_cpuset_file);
			fprintf(stderr, "reached switch case rc = %d", rc);
			if (cgroup_cpuset_file == NULL) {
				fprintf(stderr, "\n3 file is null\n");
			}
			readCgroupSubsystemFile(portLibrary, cgroup_cpuset_file, cgroupElement->value);
			fprintf(stderr, "read file and content - %s", cgroupElement->value);
